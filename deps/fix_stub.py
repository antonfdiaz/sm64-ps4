	...
	def load(self, f):
		...
		if self.shdr_size > 0:
			...
			for shdr in self.shdrs:
				shdr.name = self.strtable_data[shdr.name:].split(b'\0', 1)[0].rstrip(b'\0').decode('ascii')
		else:
...
with open(prx_file_path, 'r+b') as f:
	...
	for i, symbol in enumerate(symbols):
		name = symbol_strtable_data[symbol.name:].split(b'\0', 1)[0].rstrip(b'\0').decode('ascii')
		if len(name) == 0:
			continue
